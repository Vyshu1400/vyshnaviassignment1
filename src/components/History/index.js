...
import HistotyItem from '../HistoryItem'
import './index.css'
...
class History extends Component{
    ...
    render(
        const {searchInput,HistoryList,isShow}=this.setState
        ...
        <div>
            <ul className="history-container">
                {searchResults.length === 0?(
                    <p className="error">There is no hostory to show</p>
                ) : (
                    searchResults.map(eachHistory=>(
                        <HistotyItem
                        key={eachHistory.id}
                        historydetails={eachHistory}
                        updtaeSearch={this.updtaeSearch}
